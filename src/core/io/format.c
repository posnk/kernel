...
int core_ivprintf (	printf_helper_t __helper,
					uint32_t impl,
					const char* str, 
					va_list args)
{
...
int core_sprintf_helper(char tok, uint32_t impl)
{
	char **strp = (char **) impl;
	(**strp) = tok;
	(*strp)++;
...
int core_printf(const char* str,...){
	va_list args;
	va_start(args,str);
	int res = vsprintf(earlycon_printf_buffer, str, args);
	core_debug(earlycon_printf_buffer);
	va_end(args);
	return res;
}
